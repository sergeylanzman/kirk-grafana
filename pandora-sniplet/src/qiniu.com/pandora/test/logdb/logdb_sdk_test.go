...
func cleanupEnv() {
	...
	if err != nil {
		fmt.Println("list repo failed, err: ", err)
		return
	}
	for _, repo := range listOutput.Repos {
		err = client.DeleteRepo(&DeleteRepoInput{RepoName: repo.RepoName})
		if err != nil {
			fmt.Println("delete repo failed, err: ", err, repo.RepoName)
		}
	}
	fmt.Println("cleanup env success!")
